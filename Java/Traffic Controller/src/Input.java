...
public class Input {
    ...
    public static int getInt(String prompt) {
		...
		while (true) {
			if (sc.hasNextInt()) // if the data in the buffer is a valid integer
				break;           // then break out of the validation loop
			
			System.out.printf("Invalid input! Please enter a number: ");
			sc.next();  // clear the data in the input buffer
    ...
    public static int getIntRange(String prompt, int low, int high) {
		...
		while (true) {
			if (sc.hasNextInt()) {         // if the data in the buffer is a valid integer
				userInput = sc.nextInt();  // then store the integer in userInput  
